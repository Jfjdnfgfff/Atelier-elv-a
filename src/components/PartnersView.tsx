    ...
    <div className="space-y-5 p-3 sm:p-6" dir="rtl">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            {activeTab === 'suppliers' ? (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-2xl text-xs font-black transition-all shadow-xs"
              >
        ...
        {activeTab === 'suppliers' ? (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <span className="text-[11px] text-slate-500 font-bold block mb-0.5">عدد الموردين المسجلين:</span>
              <span className="text-xl font-black text-slate-900">{suppliers.length} موردين</span>
              <span className="text-[10px] text-slate-400 block mt-0.5">أقمشة وفساتين جاهزة</span>
            </div>
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <span className="text-[11px] text-slate-500 font-bold block mb-0.5">إجمالي المشتريات المفوترة:</span>
              <span className="text-xl font-black text-slate-900 font-mono">{totalSupplierPurchasesAll.toLocaleString()} دج</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <span className="text-[11px] text-slate-500 font-bold block mb-0.5">عدد الخياطات في الفريق:</span>
              <span className="text-xl font-black text-slate-900">{seamstresses.length} خياطات</span>
              <span className="text-[10px] text-slate-400 block mt-0.5">ورشة وتفصيل خارجي</span>
            </div>
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <span className="text-[11px] text-slate-500 font-bold block mb-0.5">إجمالي الطلبيات المسندة:</span>
              <span className="text-xl font-black text-slate-900 font-mono">{totalTailoringOrdersCount} طلبية</span>
            ...
            <div className="bg-blue-50 p-3.5 rounded-2xl border border-blue-200">
              <span className="text-[11px] text-blue-700 font-bold block mb-0.5">طلبيات قيد الإنجاز بالورشة:</span>
              <span className="text-xl font-black text-blue-800 font-mono">{totalActiveTailoringCount} طلبية</span>
              <span className="text-[10px] text-blue-600 block mt-0.5">قيد العمل أو بانتظار التسليم</span>
            </div>
            <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
              <span className="text-[11px] text-slate-500 font-bold block mb-0.5">إجمالي تكاليف ومستحقات الخياطة:</span>
      ...
      <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-2xs">
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-xl pr-10 pl-3 py-2.5 text-xs font-medium text-slate-800 focus:bg-white focus:border-blue-500 focus:outline-none"
          />
        ...
        <div className="space-y-3">
          {filteredSuppliers.length === 0 ? (
            <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center text-slate-400 space-y-2">
              <p className="text-sm font-bold text-slate-600">لا يوجد موردين مطابقين للبحث</p>
              <button
                onClick={() => {
                  setEditingSupplier(null);
                  setShowAddSupplierModal(true);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
              {filteredSuppliers.map(sup => (
                <div
                  key={sup.id}
                  className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs hover:border-blue-300 transition-all flex flex-col justify-between space-y-4"
                >
                  <div className="space-y-2.5">
                    ...
                    <div className="text-xs text-slate-500 space-y-1">
                      {sup.phone && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-mono font-bold text-slate-700" dir="ltr">{sup.phone}</span>
                        </div>
                      )}
                      {sup.addressOrCity && (
                        <div className="flex items-center gap-1.5">
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-2xl border border-slate-100 text-center text-xs">
                      ...
                      <div>
                        <span className="text-[10px] text-slate-400 block font-bold">إجمالي السلعة</span>
                        <span className="font-bold text-slate-900">{sup.totalBilled.toLocaleString()}</span>
                      </div>
                      <div>
      ...
      {activeTab === 'seamstresses' && (
        <div className="space-y-3">
          {filteredSeamstresses.length === 0 ? (
            <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center text-slate-400 space-y-2">
              <p className="text-sm font-bold text-slate-600">لا توجد خياطات مطابقة للبحث</p>
              <button
                onClick={() => {
                  setEditingSeamstress(null);
                  setShowAddSeamstressModal(true);
                  ...
                  <div className="space-y-2.5">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        ...
                        <h3 className="font-black text-slate-900 text-base mt-1">{seam.name}</h3>
                      </div>
                      {seam.activeOrders > 0 ? (
                        <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[11px] font-black px-2.5 py-1 rounded-xl">
                          {seam.activeOrders} طلبيات جارية
                        </span>
                      ) : (
                        <span className="bg-slate-100 text-slate-600 text-[11px] font-bold px-2 py-0.5 rounded-xl">
                          مكتملة
                        </span>
                      )}
                    </div>
                    ...
                    <div className="text-xs text-slate-500 space-y-1">
                      {seam.phone && (
                        <div className="flex items-center gap-1.5">
                          <Phone className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                          <span className="font-mono font-bold text-slate-700" dir="ltr">{seam.phone}</span>
                        </div>
                      )}
                      {seam.addressOrCity && (
                        <div className="flex items-center gap-1.5">
                      ...
                      {seam.ratePerPieceOrSalary && (
                        <div className="text-[11px] font-medium text-slate-700">
                          نظام التسعير: <span className="font-bold text-blue-600">{seam.ratePerPieceOrSalary}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2.5 rounded-2xl border border-slate-100 text-center text-xs">
                      <div>
                        <span className="text-[10px] text-slate-400 block font-bold">إجمالي الطلبيات</span>
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-900">{seam.totalTailorCost.toLocaleString()} دج</span>
                      </div>
                    </div>
                  </div>
      ...
      {showAddSupplierModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-xs" dir="rtl">
          <div className="bg-white rounded-3xl w-full max-w-lg p-5 sm:p-6 shadow-2xl border border-slate-100 space-y-4 max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveSupplier} className="space-y-3.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-700 block mb-1">
                    رقم الهاتف <span className="text-slate-400">(أرقام فقط)</span>
                  </label>
                  <NumbersInput
                    value={supPhone}
      ...
      {selectedSupplierHistory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-slate-900/60 backdrop-blur-xs" dir="rtl">
          <div className="bg-white rounded-3xl w-full max-w-2xl p-5 sm:p-6 shadow-2xl border border-slate-100 space-y-4 max-h-[92vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <div>
                      ...
                      <div key={order.id} className="py-3 flex flex-col sm:flex-row justify-between sm:items-center gap-2 text-xs">
                        ...
                        <div className="text-right sm:text-left shrink-0">
                          ...
                          <div className="text-[11px] text-blue-700 font-bold">
                            أتعاب الخياطة: {order.cost.toLocaleString()} دج
                          </div>
            ...
            <form onSubmit={handleConfirmSettle} className="space-y-3">
              ...
              <div>
                ...
                <NumbersInput
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 font-mono focus:bg-white focus:border-blue-500 focus:outline-none"
                  required
                />
              </div>
