...
export const RentalModal: React.FC<RentalModalProps> = ({ 
  ...
  const [rentDiscount, setRentDiscount] = useState<number | ''>(rental?.discountAmount ? rental.discountAmount : '');
  const safeDiscount = Math.min(Math.max(0, Number(rentDiscount) || 0), safeDressPrice + safeAccPrice);
  const totalRentPrice = safeDressPrice + safeAccPrice - safeDiscount;
  const [bookingDateState, setBookingDateState] = useState<string>((rental?.bookingDate || today).slice(0, 10));
  const [handoverDateState, setHandoverDateState] = useState<string>((rental?.handoverDate || today).slice(0, 10));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      bookingDate: bookingDateState || today,
      handoverDate: bookingType === 'active' ? (handoverDateState || today) : undefined,
      notes: notes.trim(),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-slate-800" dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">تاريخ الحجز / تسجيل العملية</label>
          <input
            type="date"
        ...
        {bookingType === 'active' && (
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">تاريخ تسليم الفستان للزبونة</label>
            <input
              type="date"
              value={handoverDateState}
              onChange={(e) => setHandoverDateState(e.target.value)}
              className="w-[145px] sm:w-[155px] bg-white border border-slate-200 rounded-xl px-2.5 py-2 text-xs sm:text-sm font-bold text-slate-900 focus:border-blue-500 focus:outline-none"
            />
          </div>
        )}
        ...
        <div className="sm:col-span-3 flex flex-wrap gap-1.5">
          {[1, 2, 3, 7].map(d => (
            <button key={d} type="button" onClick={() => handleDurationChange(d)}
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border ${durationDays === d ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}>
              {d === 7 ? 'أسبوع' : `${d} ${d === 1 ? 'يوم' : 'أيام'}`}
            </button>
          ))}
          <button type="button" onClick={() => handleStartDateChange(today)}
            className="px-2.5 py-1 rounded-lg text-[11px] font-bold border bg-white text-slate-700 border-slate-200 hover:bg-slate-50">
            الاستلام اليوم
          </button>
        </div>
      </div>
      ...
      <div className="p-4 rounded-3xl border border-slate-200 bg-slate-50 space-y-3">
        ...
        <div className="flex items-center gap-2 bg-amber-50 p-2.5 rounded-xl border border-amber-200/70">
          ...
          <input
            ...
            min="0"
            value={rentDiscount}
            onChange={(e) => setRentDiscount(e.target.value === '' ? '' : Number(e.target.value))}
            placeholder="0"
          ...
          {safeDiscount > 0 && (
            <span className="text-[11px] font-mono shrink-0">
              <span className="line-through text-slate-400">{(safeDressPrice + safeAccPrice).toLocaleString()}</span>
              <span className="font-bold text-emerald-700 mr-1">← {totalRentPrice.toLocaleString()} دج</span>
            </span>
          )}
        </div>
        ...
        {hasAccessories && safeAccPrice > 0 && (
          <div className="bg-white p-2.5 rounded-xl border border-slate-200 text-xs flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              ...
              {safeDiscount > 0 && <><span className="text-slate-400 font-bold">-</span><span className="text-amber-700 font-bold">تخفيض: {safeDiscount.toLocaleString()} دج</span></>}
              <span className="text-slate-400 font-bold">=</span>
